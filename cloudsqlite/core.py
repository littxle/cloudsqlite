...
"""
Core implementation of cloudsqlite proxies
"""
...
LOG = logging.getLogger("cloudsqlite")
...
class Connection(Thread):
    def __init__(
        ...
        if loop is not None:
            warn(
                "cloudsqlite.Connection no longer uses the `loop` parameter",
                DeprecationWarning,
            )
    ...
    async def cursor(self) -> Cursor:
        """Create an cloudsqlite cursor wrapping a sqlite3 cursor object."""
    ...
    async def backup(
        ...
        *,
        ...
        Takes either a standard sqlite3 or cloudsqlite Connection object as the target.
        """
...
def connect(
    ...
    **kwargs: Any,
    ...
    if loop is not None:
        warn(
            "cloudsqlite.connect() no longer uses the `loop` parameter",
            DeprecationWarning,
        )
